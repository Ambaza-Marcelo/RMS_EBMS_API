...
@section('title')
@lang('messages.order') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('messages.order')</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title float-left">Details sur le Bon de commande No : {{$code}}</h4>
                    ...
                    <div class="data-tables">
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                    <td>{{ $orderDetail->bon_no }}</td>
                                    <td>{{ $orderDetail->date }}</td>
                                    <td>{{ $orderDetail->article->name }}</td>
                                    <td>{{ $orderDetail->article->specification }}</td>
                                    <td>{{ $orderDetail->quantity }}</td>
                                    <td>{{ $orderDetail->unit }}</td>
                                    <td>{{ number_format($orderDetail->total_value,0,',','.') }}fbu</td>
                                    <td>{{ $orderDetail->description }}</td>
                                    <td>{{ $orderDetail->created_by }}</td>
                                    <td>
