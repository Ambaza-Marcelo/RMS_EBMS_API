...
@section('title')
@lang('factures Cuisine') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('factures Cuisine')</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title float-left">Liste des factures Cuisines</h4>
                ...
                <form action="{{ route('ebms_api.checkTIN') }}" method="POST">
                    ...
                    <p class="float-right mb-2">
                            <a class="btn btn-primary text-white" href="{{ route('admin.invoice-kitchens.create') }}">AJOUT DE FACTURE</a>
                    </p>
                    ...
                    <div class="data-tables">
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            @foreach($factures as $facture)
                               <tr>
                                    <td>{{ $loop->index+1}}</td>
                                    <td><a href="{{ route('admin.invoice-kitchens.show',$facture->invoice_number) }}">{{ $facture->invoice_number }}</a>&nbsp;@if($facture->etat == 0)<span class="badge badge-warning">Encours...</span>@elseif($facture->etat ==1)<span class="badge badge-success">Validée</span>@else<span class="badge badge-danger">Annulée</span>@endif</td>
                                    ...
                                    <td>
                                        ...
                                        @if($facture->etat != -1)
                                        <a href="{{ route('admin.invoice-kitchens.imprimer',$facture->invoice_number) }}"><img src="{{ asset('img/ISSh.gif') }}" width="60" title="Télécharger d'abord le document et puis imprimer"></a>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('invoice_kitchen.validate'))
                                        ...
                                        @if (Auth::guard('admin')->user()->can('invoice_kitchen.validate'))
                                        @if($facture->etat == 0)
                                         <a class="btn btn-primary text-white" href="{{ route('admin.invoice-kitchens.validate', $facture->invoice_number) }}"
                                            onclick="event.preventDefault(); document.getElementById('validate-form-{{ $facture->invoice_number }}').submit();">
                                                Valider
                                            </a>

                                            <form id="validate-form-{{ $facture->invoice_number }}" action="{{ route('admin.invoice-kitchens.validate', $facture->invoice_number) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if($facture->etat == 0)
                                         <a class="btn btn-primary text-white" href="{{ route('admin.invoice-kitchens.reset', $facture->invoice_number) }}"
                                            onclick="event.preventDefault(); document.getElementById('reset-form-{{ $facture->invoice_number }}').submit();">
                                                Annuler
                                            </a>

                                            <form id="reset-form-{{ $facture->invoice_number }}" action="{{ route('admin.invoice-kitchens.reset', $facture->invoice_number) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('invoice_kitchen.delete'))
                                         <a class="btn btn-danger text-white" href="{{ route('admin.invoice-kitchens.destroy',$facture->invoice_number) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $facture->invoice_number }}').submit();">
                                                Supprimer
                                            </a>

                                            <form id="delete-form-{{ $facture->invoice_number }}" action="{{ route('admin.invoice-kitchens.destroy',$facture->invoice_number) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
